...
function [I, limits, map] = sc2(I, varargin)
%% Check for arguments
if nargin == 0
...
%% Prob_jet
    case 'prob_jet'
        ...
        [I, limits] = real2rgb(I, cmap2, limits);
        I(repmat(M, [1 1 3])) = 0;
       % I = rescale(A + I, [0.2 1.8]); %comment out for alpha?
         I = rescale(A + I, [0 1]);
        %% Gray_jet
    case 'gray_jet'
    ...
    case 'gray_devon'
        ...
        map = load('C:\Users\abhogal\AppData\Local\Temp\Rar$DRa0.047\devon\devon.mat'); %map(1,:) = NaN; map(end,:) = NaN;
        J = real2rgb(I(:,:,1), map, limits);
        G = real2rgb(I(:,:,2), 'gray');
        A = real2rgb(I(:,:,3), 'gray');
        I = J .* A + G .* (1 - A);
