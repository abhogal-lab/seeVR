...
for ii=1:size(HRFon,1)
    ...
    for jj=1:size(HRFdsp,1) %skips the first set to make space for the original probe
        HRF_freq = fftshift(fft(HRFdsp(jj,:)/trapz(HRFdsp(jj,:))));
        tmp = real(ifft(ifftshift(HRF_freq(1,end:-1:1).*onfreq)));
        %tmp = abs(ifft(HRF_freq(1,end:-1:1).*onfreq));
        tmp2 = (tmp/trapz(tmp)); %normalize HRF
        HRF((ii)*pp+jj+1,:) =  tmp2(1,end:-1:1);
        idx((ii)*pp+jj+1,:,:,:) = [ii jj 1];
    end
end
