...
if isfield(opts,'seqpath')
    try
        disp('opts.seqpath specified... attempting to load traces')
        cd(opts.seqpath)
        oversample = 1;
        ...
        error('check path to RAMR-GEN3 breathing traces')
    end
else
    error('please specify opts.seqpath --> pointing to RAMR-GEN3 raw/bbb files')
end
